...
- [x] 教学教程（按章节递进，全部用母语代码示例）——即《中文仓颉程序设计》，已统一为项目唯一教程（2026-09 删除 docs/tutorial/ 前身目录）
- [x] 错误信息字典附录（供学习者反查）
...
## 17. AI 辅助（用户可接入：zhc ai / zhc translate）

> 用户目标（2026-09）：① 自动翻译第三方库，并把翻译结果选择性共享给他人；
> ② 按自然语言需求自动写中文仓颉代码（自动编译验证迭代）。
> 形态拍板：zhc 新子命令；模型服务 OpenAI 兼容 API 与 Ollama 都支持。
> 传输层实测结论：std.net 1.0 仅 socket 层（无 HTTP/TLS 客户端）→ LLM 调用走外部进程：
> curl（https POST，优先）→ 缺 curl 自动降级 python3 urllib.request；两者皆无则明确报错。
> 开发机无外网/无 Ollama 时，acceptance 以本地 mock OpenAI 端点（tools/mock_llm.py）驱动全链路。

### 17.1 AI 后端配置（两命令共用）

统一 OpenAI 兼容 chat completions 协议（Ollama 亦暴露 `/v1/chat/completions`，单代码路径）：

| 配置项 | 环境变量 | `~/.zhc/ai.toml` | 默认 |
|---|---|---|---|
| API 基址 | `ZHC_AI_BASE` | `[ai] base = "..."` | `https://api.openai.com/v1` |
| 密钥 | `ZHC_AI_KEY` | `[ai] key = "..."` | 空（本地端点可免） |
| 模型 | `ZHC_AI_MODEL` | `[ai] model = "..."` | 空 → 明确报错引导设置（不猜模型名） |

优先级：环境变量 > ai.toml > 默认。密钥不进 git/日志/提示词；ai.toml 生成时提示 `chmod 600`。
加载失败仅 AI 命令报错，不拖累其他命令。

### 17.2 zhc translate <库目录> [--share [导出名]]（自动翻译第三方库）

把第三方仓颉库的公开 API 翻译成中文方言映射（crates 映射，语言包 crates/ 最后加载覆盖）：

1. **提取带上下文的公开 API**：扫描 .cj 源码（复用 mapping auto 的注释剥离/逐行声明扫描），
   每条 public 标识符附带「前导 `//` 注释 + 声明行原文」作为 LLM 翻译依据；
2. **LLM 批量润色**：方言关键字冲突表 + 全部条目（官方名 | 上下文）一次请求 → 要求返回
   `{"官方名": "母语名"}` JSON；响应容错（剥 ```json 围栏、逐条容错，坏条目保留官方名）；
3. **本地质量门禁**（不依赖 LLM）：母语名撞关键字/宏/已有映射/条目间重复 → 带冲突清单
   自动重试一次，仍冲突则该条恒等保留并警告（mapping check 五项门禁仍兜底）；
4. **模块路径翻译**：库含子目录/多文件时，把相对路径段批量交给 LLM 译为母语段，写入
   `["模块路径"]` 节（import 路径段级映射语义同 module_paths.toml）；
5. **产出**：`crates/<库名>.toml`（生成于语言包根；`--install` 语义同 mapping auto）；
6. **选择性共享（--share [导出名]）**：仅在用户显式加 `--share` 时导出共享目录
   `zhc-共享-<导出名>/`（缺省导出名 = 库名；导出名含 `/` 或 `..` 时回退库名）。内含
   `lang-packs/<代码>/crates/<库名>.toml`（语言跟随当前方言代码）+ README 共享说明
   （界面语言跟随）：接收方把 `lang-packs/` 并入 `~/.zhc/`（用户目录覆盖内置，§6.3）
   或任意语言包根即生效，`zhc mapping check` 验证。不共享 = 不导出，映射仅留本机。

### 17.3 zhc ai "<需求>" [-o 输出.zc] [--iter N]（需求 → 中文仓颉代码）

1. **方言知识注入**：提示词携带关键字映射全表 + stdlib 高频别名（打印行/打印/读取行等，
   教程附录 B 高频子集），约束 LLM 只写方言标识符（否则产物无法通过 zhc 转译）；
2. **生成 → 自验证闭环**：LLM 产码 → 写入临时 `.zc` 后自调用当前可执行文件
   `zhc check`（子进程完整复用转译 + cjc 编译 + 母语诊断管线，与用户实际体验一致）→
   失败时把**母语诊断文本**回喂 LLM（只要求输出修正后完整代码）→ 重试，最多 `--iter N` 轮
   （默认 3，上限 10）；
3. **输出**：成功或耗尽后写 `-o` 文件（默认 `ai-输出.zc`）+ 报告（轮数/最终状态）；
   耗尽时仍落盘最后一份代码供人工修改；
4. **容错**：剥 ```cangjie/``` 代码块围栏；空响应自动重试不占迭代计数。

### 17.4 公共模块（ai_client.cj）

- `loadAiConfig(): AiConfig`（env > ai.toml > 默认；TomlParseError 静默 → 默认值）；
- `llmChat(cfg, messages): String`——curl 或 python3 子进程 POST
  `{base}/chat/completions`，解析 `choices[0].message.content`（std.json）；非 2xx 报错含状态码与
  响应摘要（key 无效/限流等可诊断）；超时 120s；
- `parseJsonObj(text): HashMap<String,String>`（剥围栏容错）。

### 17.5 验收（acceptance 段 17，mock 驱动不依赖外网）

tools/mock_llm.py 本地起 OpenAI 兼容端点，两场景预设响应：

- **translate 场景**：返回预设中文映射 JSON（含一个故意撞关键字的条目，验证冲突重试降级）；
  断言 crates/<库>.toml 生成 + 冲突条目恒等保留 + `--share` 目录结构齐全；
- **ai 场景**：第一次调用返回编译必败代码（类型不匹配等硬错误——实测仓颉漏分号
  与缺 import 不报错，不构成必败代码），第二次返回正确代码——断言迭代修复
  闭环真实生效（产物可运行 + 报告轮数 = 2）。
