...
2. 获取 `.vsix`，两种途径任选：
   ...
   **Linux / macOS（bash），仓库根执行**：
   ...
   ```bash
   bash tools/vscode-extension/build-vsix.sh   # 产物：tools/vscode-extension/zhc-dialect-0.1.1.vsix
...
也可临时使用：把本目录复制到 `~/.vscode/extensions/zhc-dialect-0.1.1/` 后重启 VS Code。
