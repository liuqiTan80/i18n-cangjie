...
仓颉方言编程框架（zhc）的 VS Code 配套扩展：**8 种方言**（中文 `.zc` + 英/俄/日/韩/法/西/德）的一站式编辑体验。
...
| 能力 | 说明 |
|---|---|
| **语法高亮** | 8 种方言文件按各自语言包着色（关键字/类型/函数/宏分节），语法文件由 [tools/gen_highlight.py](../gen_highlight.py) 从语言包自动生成（scopeName 按语言唯一），与映射永不漂移 |
...
| **词表联想补全** | 方言词（或官方名）即打即联想：中文文件敲「打」→ 打印/打印行…，俄语文件敲「печ」→ печать…；词条与语言包同源（tools/gen_words.py 生成 zhc-words.json + words-<code>.json）。**函数类选中自动带括号且光标居中**，**类型/关键字/字面量/模块路径**分类显示；敲 `@` 弹出宏词条（中文 `@派生`/`@测试`/`@期望`，日语 `@テスト`…），中/日/韩输入法上屏同样触发（VS Code 对上屏文本不自动弹补全，扩展手动拉起） |
...
3. 打开任意方言文件（`.zc`/`.en`/`.rc`/`.jc`/`.kc`/`.fc`/`.sc`/`.dc`）即自动激活；非默认扩展名文件可用右下角语言模式手动切换（如 `.rc` 被当作其他语言打开时选 `zhc-ru`）。
...
```bash
# 重新生成语法高亮/词表（语言包更新后执行；无参数 = 全部 8 语言批量生成）
...
node test/words.test.js        # 词表：前缀联想/双向查/分类/多语言词表（8 项）
```
...
| 其余 7 语言 | `syntaxes/zhc-<code>.tmLanguage.json`（scope `source.zc.<code>`） | `lib/words-<code>.json` |

scopeName 按语言唯一是必须的：VS Code TextMate 注册表按 scopeName 全局去重，若 8 语言共用 `source.zc`，后加载的语法文件会覆盖先加载者，全部语言将按同一份词表着色。
